...
class TripleEmbedder(nn.Sequential):
    """
    ...
    def __init__(self, model_name_or_path: Optional[str] = None, modules: Optional[Iterable[nn.Module]] = None,
        ...
        if model_name_or_path is not None and model_name_or_path != "":
            ...
            basic_transformer_models = ['albert-base-v1', 'albert-base-v2', 'albert-large-v1', 'albert-large-v2',
                                        ...
                                        'albert-xxlarge-v2', 'bert-base-cased-finetuned-mrpc',
                                        'bert-base-cased', 'bert-base-chinese', 'bert-base-german-cased',
                                        'bert-base-german-dbmdz-cased', 'bert-base-german-dbmdz-uncased',
                                        'bert-base-multilingual-cased', 'bert-base-multilingual-uncased',
                                        'bert-base-uncased', 'bert-large-cased-whole-word-masking-finetuned-squad',
                                        ...
                                        'distilbert-base-uncased', 'distilgpt2', 'distilroberta-base', 'gpt2-large',
                                        'gpt2-medium', 'gpt2-xl', 'gpt2', 'openai-gpt', 'roberta-base-openai-detector',
                                        'roberta-base', 'roberta-large-mnli', 'roberta-large-openai-detector',
                                        'roberta-large', 't5-11b', 't5-3b', 't5-base', 't5-large', 't5-small',
                                        'transfo-xl-wt103', 'xlm-clm-ende-1024', 'xlm-clm-enfr-1024',
                                        'xlm-mlm-100-1280', 'xlm-mlm-17-1280', 'xlm-mlm-en-2048', 'xlm-mlm-ende-1024',
                                        ...
                                        'xlm-roberta-large', 'xlnet-base-cased', 'xlnet-large-cased']
        ...
        self.triple_mapper = modules[str(0)]

    def map_triples(self, _idx1, _idx2):
        a, b = self.triple_mapper(_idx1, _idx2)
        return a, b
    ...
    def smart_batching_collate(self, batch):
        ...
        for idx in range(num_texts):
            tokenized = self.tokenize(texts[idx])[0]
            #batch_to_device(tokenized, self._target_device)
            sentence_features.append(tokenized)
        sentence_features = torch.stack(sentence_features)
        return texts, labels
    ...
    def _text_length(self, text: Union[List[int], List[List[int]]]):
        ...
        if isinstance(text, dict):              #{key: value} case
            ...
            return 1
        elif text.dim() == 0 or isinstance(text[0], int):    #Empty string or list of ints
            return text.dim()
        else:
    ...
    def fit(self,
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _s in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    if use_amp:
                        ...
                        feat_a = torch.stack(features[0]).cuda()
                        feat_b = torch.stack(features[1]).cuda()
                        emb1, emb2 = self.map_triples(feat_a, feat_b)
                        labs = labels.cuda()
                        #print(feat_a.shape)
                        #print(feat_b.shape)
                        #print(labs.shape)
                        loss_value = loss_model(emb1, emb2, labs)
                        #if _s % 100 == 0:
                        #    print(loss_value)
                        loss_value.backward()
                        torch.nn.utils.clip_grad_norm_(loss_model.parameters(), max_grad_norm)
