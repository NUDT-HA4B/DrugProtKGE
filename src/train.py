...
def fetch_embeddings(_path):
    mod = torch.load(_path, map_location=torch.device('cuda'))
    #mod = torch.load(_path, map_location=torch.device('cpu'))
...
def run_triple_fitting(_mod_name, _feature_name, _ds_name):
 ...
 if __name__ == "__main__":
     run_triple_fitting('conve', 'ht', 'ChemProt')